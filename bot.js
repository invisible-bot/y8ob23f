        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                if (conn.user.jid === '@s.whatsapp.net') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*පම්කයා public ආකාරයට ක්‍රියාකිරිම ආරම්භ විය.👸*\n\n_කරුණාකර මෙහි command උත්සාහ නොකරම්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ command list එක ලබාගැනීමට .panel command එක භාවිතා කරම්න.*\n\n*ඔබේ bot public ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.setvar WORK_TYPE:private_ *විධානය භාවිතා කරම්න.*\n\n*Updates:* https://gist.github.com/BlackAmda/890b6b31fcb8d376d6a68afcb7359324\n\n*ඇම්ඩන් භාවිතා කිරීම ගැන ස්තූතියි 💌*', MessageType.text);
                }
            }
            else {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                ...
                await conn.sendMessage(conn.user.jid, '*පම්කයා private ආකාරයට ක්‍රියාකිරිම ආරම්භ විය.👸*\n\n_කරුණාකර මෙහි command උත්සාහ නොකරම්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ command list එක ලබාගැනීමට .panel command එක භාවිතා කරම්න.*\n\n*ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.setvar WORK_TYPE:public_ *විධානය භාවිතා කරම්න.Updates:* https://gist.github.com/BlackAmda/890b6b31fcb8d376d6a68afcb7359324\n\n*\n\n*ඇම්ඩන් භාවිතා කිරීම ගැන ස්තූතියි 💌*', MessageType.text);
                }
            }
            else {
                ...
                if (conn.user.jid === '@s.whatsapp.net') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*පම්කයා private ආකාරයට ක්‍රියාකිරිම ආරම්භ විය.👸*\n\n_කරුණාකර මෙහි command උත්සාහ නොකරම්න. මෙය ඔබගේ ලොග් අංකයයි._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය :)_\n\n*ඔබේ command list එක ලබාගැනීමට .panel command එක භාවිතා කරම්න.*\n\n*ඔබේ bot private ක්‍රියාත්මක වේ. වෙනස් කිරීමට* _.setvar WORK_TYPE:public_ *විධානය භාවිතා කරම්න.*\n\nUpdates:* https://gist.github.com/BlackAmda/890b6b31fcb8d376d6a68afcb7359324\n\n*ඇම්ඩන් භාවිතා කිරීම ගැන ස්තූතියි 💌*', MessageType.text);
                }
            }
        }
        else {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- දෝෂ වාර්තාව [පම්කයා UOR-FoE 21] --*\n\n' + 
                                    ...
                                    '\n_https://chat.whatsapp.com/G8OlAU6LPC63HoaVhsIGmF ඔබට එය මෙම group යොමු කළ හැකිය._\n\n' +
                                    '*සිදු වූ දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false});
                            } else {
                                await conn.sendMessage(conn.user.jid, '*-- බොට් වාර්තාව [පම්කයා UoR-FoE 21] --*\n' + 
                                    '\n*බොට් නිසි ලෙස ක්රියා කරම්න.*\n'+
                                    '\n_Message logs ඔබගේ අංකය හෝ ප්‍රති පාර්ශ්වයේ අංකය අඩංගු විය හැකිය. කරුණාකර එය සමග සැලකිලිමත් වන්න!_\n' +
                                    '\n_උදව් සඳහා ඔබට අපගේ whatsapp support කණ්ඩායමට ලිවිය හැකිය._\n' +
                                    '\n_(සුරකින ලද පණිවිඩ)_\n\n' +
                                    '\n_ඔබේ bot සඳහා යම් උදව්වක් අවශ්‍ය නම්, https://chat.whatsapp.com/G8OlAU6LPC63HoaVhsIGmF\n\n' +
                                    '*Report:* ```' + error + '```\n\n'
                                    , MessageType.text);
                            }
                        }
